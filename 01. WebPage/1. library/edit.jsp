...
<%
	...
	String pwch = request.getParameter("pw_ch");
%>
<% 
Connection con = null;
...
// 로그인 기능 참고 url : https://happy-inside.tistory.com/entry/JSP-JSP-%EA%B2%8C%EC%8B%9C%ED%8C%90-%EB%A7%8C%EB%93%A4%EA%B8%B0-4%EA%B0%95-%EB%A1%9C%EA%B7%B8%EC%9D%B8-%EA%B8%B0%EB%8A%A5-%EA%B5%AC%ED%98%84%ED%95%98%EA%B8%B0
	try {
		...
		String sql1 = "update student SET passwd = ? where id = ?";
		pstmt = con.prepareStatement(sql);
		pstmt1 = con.prepareStatement(sql1);
	
		pstmt.setString(1, id); // 첫번째 물음표에 매개변수로 받아온 id를 대입한다.
		pstmt1.setString(1, pwch); // 두번째 물음표에 매개변수로 받아온 id를 대입한다.
		pstmt1.setString(2, id); // 세번째 물음표에 매개변수로 받아온 id를 대입한다.
		...
		if (rs.next()) {
			if(rs.getString(1).equals(passwd)){
				...
				response.sendRedirect("http://localhost:8080/MyJspPage/editok.jsp?name=" + request.getParameter("id")
...
<p style="color:red"> 비밀번호 변경을 실패하였습니다. 아이디와 비밀번호를 다시 확인해주세요. </p>

<a href="logout.html"> <div align="right"">로그아웃</div></a>
...
<form method="post" action="edit.jsp">
	...
	<p id="fsize">변경할 비밀번호</p>
	<input type="password" name="pw_ch"id="box"></p>
	
	<input type="submit" value="비밀번호 변경" style="height:50px; width:100px;background-color:red;">
	<button type="button" onclick="location.href='mypage.html'"style="height:50px; width:100px;background-color:skyblue;" >돌아가기</button >
</form> 
</div>
</center>
